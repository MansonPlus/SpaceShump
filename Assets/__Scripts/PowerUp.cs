...
public class PowerUp : MonoBehaviour
{
    ...
    private BoundsCheck bdnCheck;
    private Material cubeMat;

    void Awake() {
        // Find tge Cube reference (there's only a single child)
        cube = transform.GetChild(0).gameObject;
        // Find the TextMesh and other components
        letter = GetComponent<TextMesh>();
        rigid = GetComponent<Rigidbody>();
        bdnCheck = GetComponent<BoundsCheck>();
        cubeMat = cube.GetComponent<Renderer>().material;

        // Set a random velocity
        Vector3 vel = Random.onUnitSphere; // Get Random XYZ velocity
        vel.z = 0; // Flatten the vel to the XY plane
        vel.Normalize(); // Normalizing a Vector3 set its length to 1m
    ...
    void Update() {
        ...
        // If u>0, decrease the opacity (i.e., alpha) of the PowerUp & Letter
        if (u > 0) {
    ...
    public void SetType(eWeaponType wt) {
        ...
        cubeMat.color = def.powerUpColor;// projectileColor; // Set the color of the PowerCube
    ...
    public void AbsorbeBy(GameObject target) {
        Destroy(this.gameObject);
    }
}